...
namespace PhoneBook
{
    ...
    bool PhoneBookVector::search( /* In */ const std::string& first, /* In */ const std::string& last, /* Out */ std::size_t& number) const
    {
        MatchName name(first, last);

        // Stilistik:
        // STL: Programming in Building Blocks
        // Den Zugriff auf einen Container
        // Das Traversieren eines Containers
        // ABER: Keine Verwendung von Indices // 0,1, 2, max, Kein < oder <= Problem
        std::vector<Entry>::/*iterator*/ const_iterator pos = std::find_if(
            m_entries.begin(),
            m_entries.end(),
            name
        );
    ...
    {
        MatchName name(first, last);    // Aufrufbares Objekt erstellen - mit Vor- und Nachname
    ...
    void PhoneBookVector::printEntry(const Entry& entry)
    {
        ...
        std::println("{} {}: {}", entry.m_first, entry.m_last, entry.m_number);
    }
    ...
    void PhoneBookVector::print() const
    {
        std::for_each(
            m_entries.begin(),
            m_entries.end(),
            printEntry   // ist eine Klassen Methode (!!!) der Klasse, kein Member 
        );
    }
}
